...
***
# 核心思想：从解释器到编译器
Karpathy 的核心洞察是：传统 RAG 像**解释器**——每次查询时从零检索、重新理解、生成回答，知识无法积累。LLM Wiki 则像**编译器**——在摄入资料时就将其"编译"为持久化的结构化 Wiki，知识被编译一次后持续沉淀，实现复利式增长。
关键对比：RAG 是 amnesiac（健忘的），每次查询等于重新开始；LLM Wiki 是 compounding（复利的），每篇新资料都会让整个知识库变得更丰富。一个核心动作可能触发 10-15 个 Wiki 页面的级联更新。
***
# 三层架构
