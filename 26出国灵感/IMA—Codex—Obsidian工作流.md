...
verified: 2026-07-13
---
首先：workbuddy、TRAE、Qoder等国产agent也能完成。且workbuddy对办公软件的适配更好，值得优先使用。TRAE作为兜底。重活使用翻墙+Codex官方。且一般不使用Codex，因为太重了且某些纯办公能力没有适配好。只有重大科研任务才出动Codex。Codex平时不用时可考虑租给别人。
回国后不再使用claude code，一是防止被封号，二是太贵了。
# 核心构想
...
9. 用户在 Obsidian 中阅读、修改、回顾和复习，形成**长期可积累的个人知识**。
```mermaid
flowchart TD
